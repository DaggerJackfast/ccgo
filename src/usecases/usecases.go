...
func (interactor *OrderInteractor) Items(userId, orderId int) ([]Item, error) {
	...
	if user.Customer.Id != order.Customer.Id {
		message := "User #%d (customer #%d) is not allowed to see items in order  #%d (of customer #%d)"
...
func (interactor *OrderInteractor) Add(userId, orderId, itemId int) error {
	...
	order := interactor.OrderRepository.FindById(orderId)
	if user.Customer.Id != order.Customer.Id {
		message = "User #%d (customer #%d) is not allowed to add items to order %d (of customer #%d)"
		err := fmt.Errorf(message, user.Id, user.Customer.Id, order.Id, order.Customer.Id)
		interactor.Logger.Log(err.Error())
		return err
	}
	...
	if domainErr := order.Add(item); domainErr != nil {
		message = "Could not add item #%d to order #%d (of customer #%d) as user #%d because a business rule was violated: '%s'"
		err := fmt.Errorf(message, item.Id, order.Id, order.Customer.Id, user.Id, domainErr.Error())
		return err
	}
	interactor.OrderRepository.Store(order)
	interactor.Logger.Log(fmt.Sprintf("User added item '%s' (#%d) to order #%d", item.Name, item.Id, order.Id))
	return nil
}
...
func (interactor *AdminOrderInteractor) Add(userId, orderId, itemId int) error {
	...
	order := interactor.OrderRepository.FindById(orderId)
	if !user.IsAdmin {
		message = "User #%d (customer #%d) is not allowed to add items to order #%d (of customer #%d), because he is not an administrator"
		err := fmt.Errorf(message, user.Id, user.Customer.Id, order.Id, order.Customer.Id)
		interactor.Logger.Log(err.Error())
		return err
	}
	item := interactor.ItemRepository.FindById(itemId)
	if domainErr := order.Add(item); domainErr != nil {
		message = "Could not to add item #%d to order #%d (of customer #%d) as user #% because a business rule was violated; '%s'"
		err := fmt.Errorf(message, item.Id, order.Id, order.Customer.Id, user.Id, domainErr.Error())
		interactor.Logger.Log(err.Error())
		return err
	}
	interactor.OrderRepository.Store(order)
	interactor.Logger.Log(fmt.Sprintf("Admin added item '%s' (#%d) to order #%d", item.Name, item.Id, order.Id))
	return nil
}
