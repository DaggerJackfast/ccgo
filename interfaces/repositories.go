...
func (repo *DbOrderRepo) Store(order domain.Order) {
	...
	for _, item := range order.Items {
		repo.dbHandler.Execute(fmt.Sprintf(`INSERT INTO order_items (item_id, order_id)
									VALUES ('%d', '%d'`, item.Id, order.Id))
	}
}
func (repo *DbOrderRepo) FindById(id int) domain.Order {
	row := repo.dbHandler.Query(fmt.Sprintf(`SELECT name FROM orders
										WHERE id = '%d' LIMIT 1`, id))
	var customerId int
	row.Next()
	row.Scan(&customerId)
	...
	row = repo.dbHandler.Query(fmt.Sprintf(`SELECT item_id FROM item_orders
										WHERE id = '%d' LIMIT 1`, id))
	for row.Next() {
		row.Scan(&itemId)
